...
const makeEventBuffer = (logger) => {
    ...
    return {
        ...
        removeAllListeners: (...args) => {
             ...
             * This prevents orphaned setTimeout callbacks, unbounded
             * historyCache growth, and stale message references.
             */
            if (bufferTimeout) {
                clearTimeout(bufferTimeout)
                bufferTimeout = null
...
function append(data, historyCache, event, eventData, logger) {
    ...
    switch (event) {
        case 'messaging-history.set':
            ...
            if (eventData.lidPnMappings) {
                data.historySets.lidPnMappings = (data.historySets.lidPnMappings || []).concat(eventData.lidPnMappings)
            }
...
function consolidateEvents(data) {
    ...
    if (!data.historySets.empty) {
        map['messaging-history.set'] = {
            ...
            lidPnMappings: data.historySets.lidPnMappings
        }
    }
